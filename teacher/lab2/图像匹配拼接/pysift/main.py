...
def Panorama_stitching(image_right, image_left, downscale_factor=8):
    """
        全景拼接函数。
    ...
    print(f"图像下采样耗时: {end - start:.4f}秒")
    ...
    print(f"灰度转换耗时: {end - start:.4f}秒")

    # 使用SIFT特征检测器
    start = time.time()
    keypoints_right, descriptors_right = pysift.computeKeypointsAndDescriptors(gray_right)  #接受一个灰度图像
    end = time.time()
    print(f"右侧图像SIFT特征检测耗时: {end - start:.4f}秒")
    
    start = time.time()
    keypoints_left, descriptors_left = pysift.computeKeypointsAndDescriptors(gray_left)
    end = time.time()
    print(f"左侧图像SIFT特征检测耗时: {end - start:.4f}秒")
    
    print(f"检测到的关键点数量 - 右侧: {len(keypoints_right)}, 左侧: {len(keypoints_left)}")
    print(f"描述符维度 - 右侧: {descriptors_right.shape}, 左侧: {descriptors_left.shape}")
    ...
    print(f"筛选好的匹配点耗时: {end - start:.4f}秒")
    ...
    if len(good_matches) > 10:
        ...
        print(f"RANSAC单应矩阵计算耗时: {end - start:.4f}秒")
        ...
        print(f"透视变换与图像拼接耗时: {end - start:.4f}秒")
        ...
        print(f"匹配点绘制与保存耗时: {end - start:.4f}秒")

        total_end = time.time()
        print(f"=== 全景拼接总耗时: {total_end - total_start:.4f}秒 ===")
        return panorama
